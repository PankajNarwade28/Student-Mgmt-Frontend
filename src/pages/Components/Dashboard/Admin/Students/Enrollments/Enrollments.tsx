...
const Enrollments: React.FC = () => {
  ...
  // 1. Pagination State & Configuration
 ...
 <div className="space-y-4 animate-in fade-in duration-500">
  ...
  {/* COMPACT ASSIGNMENT MODAL WITH PAGINATION */}
{selectedCourse && (() => {
  const itemsPerPage = 5; // Adjust this number to change density
  ...
  return (
    <div className="fixed inset-0 bg-[#004d40]/20 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-sm rounded-[1.5rem] shadow-xl overflow-hidden animate-in zoom-in duration-150">
        
        {/* Header: More compact padding */}
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-teal-50/20">
          <div>
            <h2 className="text-lg font-black text-slate-800 tracking-tight">
              Manage Enrollment
            </h2>
            <p className="text-[10px] text-teal-600 font-bold uppercase tracking-[0.15em] mt-0.5 truncate max-w-[240px]">
              {selectedCourse.name}
            </p>
          </div>
          <button
            ...
            className="p-2 hover:bg-slate-100 rounded-full transition-all text-slate-400 hover:text-red-500"
          >
            <HiOutlineX size={18} />
          </button>
        </div>

        {/* Student List: Slimmer rows and controlled height */}
        <div className="p-4 max-h-[320px] overflow-y-auto space-y-2 scrollbar-hide">
          {currentStudents.map((student) => {
            const isEnrolled = selectedCourse.enrolled_students?.some((s) => s.id === student.id);
            return (
              <div
                key={student.id}
                className="flex items-center justify-between p-3 rounded-xl border border-slate-50 bg-slate-50/40 hover:bg-white hover:shadow-sm transition-all"
              >
                ...
                >
                  {isEnrolled ? (
                    ...
                    <>
                      <HiOutlineUserAdd size={12} /> Assign
                    </>
                  )}
                </button>
              </div>
        ...
        {totalPages > 1 && (
          <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <button
              ...
              className="px-2.5 py-1 text-[10px] font-bold text-slate-500 hover:text-teal-600 disabled:opacity-40 disabled:hover:text-slate-500 transition-colors"
            >
              Previous
            </button>
